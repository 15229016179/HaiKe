...
public class PrimaryKeyUtil implements Comparable<PrimaryKeyUtil>, java.io.Serializable{
	
	/**
	 * @Fields serialVersionUID : 序列ID
	 */
	private static final long serialVersionUID = -1280665547813741264L;
	
	/**
	 * @Fields LOG : 日志组件
	 */
	private static final Log LOG = LogFactory.getLog(PrimaryKeyUtil.class);
	 ...
	 * @return the new id
	 */
	public static PrimaryKeyUtil get() {
		return new PrimaryKeyUtil();
	}
	 ...
	 * @since 2.12.0
	 */
	public static PrimaryKeyUtil createFromLegacyFormat(final int time, final int machine, final int inc) {
		return new PrimaryKeyUtil(time, machine, inc);
	}
	 ...
	 */
	@Deprecated
	public static PrimaryKeyUtil massageToPrimaryKey(Object o) {
		if (o == null)
			return null;

		if (o instanceof PrimaryKeyUtil)
			return (PrimaryKeyUtil) o;

		if (o instanceof String) {
			String s = o.toString();
			if (isValid(s))
				return new PrimaryKeyUtil(s);
		}
	 ...
	 *            the date
	 */
	public PrimaryKeyUtil(Date time) {
	 ...
	 */
	public PrimaryKeyUtil(Date time, int inc) {
	 ...
	 * @param inc
	 *            the counter
	 * @see PrimaryKeyUtil#createFromLegacyFormat(int, int, int)
	 ...
	 *             {@link org.PrimaryKeyUtil.types.PrimaryKey#PrimaryKey(byte[])} or
	 *             {@link PrimaryKeyUtil#createFromLegacyFormat(int, int, int)}
	 *             instead.
	 */
	@Deprecated
	public PrimaryKeyUtil(Date time, int machine, int inc) {
	 ...
	 */
	public PrimaryKeyUtil(String s) {
		this(s, false);
	}
	 ...
	 */
	@Deprecated
	public PrimaryKeyUtil(String s, boolean babble) {
	 ...
	 */
	@Deprecated
	public PrimaryKeyUtil(int time, int machine, int inc) {
		_time = time;
		_machine = machine;
		_inc = inc;
		_new = false;
	...
	public boolean equals(Object o) {
		...
		PrimaryKeyUtil other = massageToPrimaryKey(o);
		if (other == null)
			return false;
	...
	/**
	 * @return a string representation of the PrimaryKey in hexadecimal format
	 * @see PrimaryKeyUtil#toHexString()
	...
	public int compareTo(PrimaryKeyUtil id) {
		if (id == null)
			return -1;
	...
	static {
		...
		try {
			int machinePiece;
			{
				...
				//LOG.fatal("machine piece post: " + Integer.toHexString(machinePiece));
			}
			...
			final int processPiece;
			{
				...
				ClassLoader loader = PrimaryKeyUtil.class.getClassLoader();
				...
				//LOG.fatal("process piece: " + Integer.toHexString(processPiece));
			}

			_genmachine = machinePiece | processPiece;
			//LOG.fatal("machine : " + Integer.toHexString(_genmachine));
		} catch (Exception e) {
